...
import Tooltip from "../Molecules/Moleculescomponent";
import {
...
const WagonDetailsTable = ({ data, classType }) => {
  ...
  const isFourthOrFirstClass = classType === "fourth" || classType === "first";
  ...
  return (
    <table className="amount-seats_table">
      ...
      <thead className="amount-seats_table-header">
        <tr>
          <th className="amount-seats_table-title" title="Места">
            Места
          ...
          <th className="amount-seats_table-title" title="Стоимость">
            Стоимость
          </th>
...
const WagonServices = ({ className, data }) => {
  ...
  const onMouseOver = (event) => {
    const name = event.target.dataset.name;
    tooltipRef.current = name;
    document.querySelector(`.tooltip_${name}`)?.classList.add("visible");
  };

  const onMouseOut = (event) => {
    const name = event.target.dataset.name;
    document.querySelector(`.tooltip_${name}`)?.classList.remove("visible");
  };

  const ServiceButton = ({ option, index }) => {
    const getButtonClass = () => {
      if (option.name === "linens" && option.is_linens_included) {
        return "btn wagon-services_btn included";
      }
      return option.selected ? "btn wagon-services_btn selected" : "btn wagon-services_btn";
    };

    const isDisabled = () => {
      if (option.name === "conditioning") {
        return !option.have_air_conditioning;
      }
      if (option.name === "wifi") {
        return !option.have_wifi && !option.wifi_price;
      }
      return false;
    };
    ...
    return (
      <div className="services-button_wrap" key={option.name}>
        <button
        ...
        >
          {svgIcons[option.name]}
        </button>
        <Tooltip name={option.name} text={tooltipTexts[option.name]} />
      </div>
    );
  };
  ...
  return (
    <div className={`${className}_services-block`}>
      ...
      <div className="btn-desk services-buttons" role="group" aria-label="Basic example">
        {options.map((option, index) => (
          <ServiceButton key={option.name} option={option} index={index} />
        ))}
      </div>
    </div>
  );
...
// WagonSeats Component
const WagonSeats = ({ className, data }) => {
  const bottomSeats = data.seats.filter(item => item.index % 2 === 0);
  const topSeats = data.seats.filter(item => item.index % 2 !== 0);
