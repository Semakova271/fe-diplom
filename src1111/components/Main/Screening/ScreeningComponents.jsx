...
import TrainsMenuCard from "../../Main/SelectionTrain/TrainsMenu/TrainsMenu";
...
// Компонент экрана поезда
export const ScreenTrain = () => {
...
// Компонент экрана пассажиров
export const ScreenPassengers = ({ data = [] }) => {
  ...
  const handleEditPassengers = () => {
    navigate({
      pathname: `/fe-diplom/passengers/${params.id}`,
      ...
      <div className="card screening-block_card">
        ...
        <div className="screening-block_card-body">
          <div className="card-desk">
            {data.map((item, index) => (
              <Card key={`passenger-${index}`} className="screening-block_card-body-item">
                ...
                <CardBody className="passengers">
                  <div className="passengers_body_group-text">
                    <span className="passengers_body-text">
                      {`${item.dataPass.info.last_name} ${item.dataPass.info.first_name} ${item.dataPass.info.patronymic}`}
                    </span>
                    <span className="passengers_body-text">
                      {`Пол ${item.dataPass.info.gender === "male" ? "мужской" : "женский"}`}
                    </span>
                    <span className="passengers_body-text">
                      {`Дата рождения ${format(new Date(item.dataPass.info.date_birth), "dd.MM.yyyy")}`}
                    </span>
                    <span className="passengers_body-text">
                      {item.dataPass.docs.type_docs.id === "passport"
                        ? `${item.dataPass.docs.type_docs.description} ${item.dataPass.docs.data_docs.seria} ${item.dataPass.docs.data_docs.number}`
                        : `${item.dataPass.docs.type_docs.description} ${item.dataPass.docs.data_docs.number}`}
                    </span>
                  </div>
                </CardBody>
              </Card>
...
// Компонент экрана оплаты
export const ScreenPayment = ({ data }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const params = useParams();
  ...
  return (
    <div className="screening-block screening-block_payment">
      <div className="card screening-block_card">
        <div className="screening-block_card-top">
          <Title text="Способ оплаты" className="screening-block_card_title" />
        </div>
